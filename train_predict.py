...
def create_xgboost_features(df, holidays_df):
    ...
    if 'y' in df.columns:
        ...
        # Fill NaN values created by shifting (e.g., with mean or 0)
        df['y_lag_1'].fillna(df['y_lag_1'].mean(), inplace=True)
        df['y_lag_7'].fillna(df['y_lag_7'].mean(), inplace=True)
    else:
...
X_test.fillna(X_train.mean(), inplace=True) # Use train mean for test set
